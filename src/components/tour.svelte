<script lang="ts">
	...
	const destroyPannellum = () => {
		// @ts-ignore
		if (window.pannellum) delete window.pannellum;
		// @ts-ignore
		if (window.libpannellum) delete window.libpannellum;
	};
			...
			function Ba(K) {
				...
				function Ja(a, e, g, p, l) {
					...
					if (h) {
						...
						u ||
							((e.timestamp = c.nodeCacheTimestamp++), c.currentNodes.push(e), c.nodeCache.push(e));
						if (e.level < c.level) {
				...
				this.init = function (L, e, Ca, H, l, h, d, u) {
					...
					if (
						...
						if (0 !== H)
							throw (
								(console.log('Error: Something went wrong with WebGL!', H), { type: 'webgl error' })
							);
						d();
					}
				};
				...
				this.render = function (g, e, k, s) {
					...
					if (a || ('multires' != z && 'cubemap' != z)) {
						if ('multires' != z)
							(k =
								2 * Math.atan(Math.tan(0.5 * k) / (a.drawingBufferWidth / a.drawingBufferHeight))),
								(k = 1 / Math.tan(0.5 * k)),
								a.uniform1f(c.psi, e),
			...
			function Ba(s, k) {
				...
				function ua(a) {
					if (ha && G) {
						...
						n =
							(((180 * (Math.atan((a.y / f) * 2 - 1) - Math.atan((ya / f) * 2 - 1))) / Math.PI) *
								n) /
								90 +
							Pa;
	...
	<div id="box" class="flex items-center justify-center w-full h-full">
		{#if !pannellum}
			<div id="loading-pannellum-1" class="flex items-center justify-center">
				<svg
					class="animate-spin mr-2 h-6 w-6 text-gray-400"
